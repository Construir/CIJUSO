<div class="container col-10">
	...
	<div class="card">
		<div class="card-body">
			<h5 class="card-title">Lotes procesados</h5>			
			<br>
				...
				<?php foreach($importaciones as $importacion){?>
					<tr>
						...
						<?php }else{ ?>
							...
							<td class="text-danger">Abierto</td>
							
						<?php } ?>
						
						<td><?php if(!empty($importacion['FechaCierre'])){echo date("d-m-Y H:i:s",strtotime($importacion['FechaCierre']));}?></td>
						<td><?php echo $importacion['pendiente']?></td>
						<td><?php echo $importacion['facturada']?></td>
						<td><?php echo $importacion['total_facturas']?></td>

						<td style="text-align:center;width:8%">
							
							<div class="form-inline">							
																
								<form class="form-inline" action="ver_lote" method="post" enctype="multipart/form-data" id="formverlote" name="formverlote">				  
									<input type="hidden" name="id_lote_ver" id="id_lote_ver" value="<?php echo $importacion['Id']?>">
									<button style="margin-right:3px" type="submit" class="btn btn-primary btn-sm">
										<img src="<?php echo base_url(); ?>/public/imagenes/ver.png" width="20" height="20" title="Ver Lote"> 
									</button>
								</form>
								<?php if($importacion['Estado'] == 2){?>
									<form class="form-inline" action="cerrar_lote" method="post" enctype="multipart/form-data" id="formcerrarlote" name="formcerrarlote">				  
										<input type="hidden" name="id_lote_cerrar" id="id_lote_cerrar" value="<?php echo $importacion['Id']?>">
										<button style="margin-right:3px" type="submit" class="btn btn-danger btn-sm">
											<img src="<?php echo base_url(); ?>/public/imagenes/cancelar.png" width="20" height="20" title="Cerrar Lote">
										</button>
									</form>
								<?php }?>	
